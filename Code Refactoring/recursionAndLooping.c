...
recursionAndLooping(char** file1)
{
	printf("Searching recursion and looping:\n\n");
	...
	for (int i = 0; i < n; i++)
	{
		if (functionNames[i][0] != 0)
		{
			...
			if (r[0][0] != 0)
				printf("Recursion in function %s\n", functionNames[i]);
			for (int j = i + 1; j < n; j++)
			{
				r[0][0] = 0;
	...
	if (looping > 0)
		printf("Program includes looping(%d)\n", looping);
	else
		printf("Program does not include looping\n");
